""" little_elsa.py - Synthetic Cirq implementation.
=== Reference ===
...
class Func5(cirq.Gate):
	...
	def _decompose_(self, qubits):
		...
		if not self.is_inverse: 
			# Compute 
			yield cirq.TOFFOLI( anc[0], qubits[0], anc[1] )
...
class Func4(cirq.Gate):
	...
	def _decompose_(self, qubits):
		...
		if not self.is_inverse: 
			...
			yield cirq.TOFFOLI( qubits[1], anc[1], qubits[0] )
			# Store 
			# Note: this version does not store results.
		else: 
			# Uncompute 
...
class Func1(cirq.Gate):
	...
	def _decompose_(self, qubits):
		...
		res = [qubits[1]]
		# Non-leaf function
		func2 = Func2(6, is_inverse=self.is_inverse)
		func2R = Func2(6, is_inverse=(not self.is_inverse))
		nq0 = [anc[0], anc[1], anc[2], anc[3], anc[4], anc[5]]
		func3 = Func3(3, is_inverse=self.is_inverse)
		func3R = Func3(3, is_inverse=(not self.is_inverse))
		nq1 = [qubits[1], anc[0], anc[6]]
		func4 = Func4(4, is_inverse=self.is_inverse)
		func4R = Func4(4, is_inverse=(not self.is_inverse))
		nq2 = [qubits[1], anc[1], anc[7], anc[8]]
		if not self.is_inverse: 
			# Compute 
			yield cirq.CNOT( anc[1], qubits[1] )
			yield cirq.CNOT( anc[0], anc[1] )
...
def main():
	...
	# Start computation
	func1 = Func1(11, is_inverse=False)
	circ = func1(*all_qubits)
	func1R = Func1(11, is_inverse=True)
